...
Твоя задача — помогать клиентам, используя только информацию ниже. Не показывай прямые ссылки в тексте ответа, используй кнопку «Перейти на сайт».
...
BUTTON_LINKS = {
    ...
    "🚚 Сроки и доставка": "https://borisov.store/offer/#3.7",
    ...
    "⚙️ Технологии": SITE_URL,   # ведёт на главную, подробности в тексте
...
async def handle_question(message: types.Message):
    ...
    # Если нажали «⚙️ Технологии» – сразу отдаём подготовленный текст, не дёргая нейросеть
    ...
    # Для остальных сообщений – ответ через нейросеть
    try:
        response = await client.chat.completions.create(
            model="google/gemini-2.5-flash-lite",
            messages=[
                {"role": "system", "content": f"{KNOWLEDGE}\n\nОтвечай без прямых ссылок. Направляй клиента к кнопке «Перейти на сайт» для подробностей."},
